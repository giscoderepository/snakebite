...
> {
  ...
  optionChangeSave = (prop: string, value: any) => {
    const currentLayer = this.props.config.layersConfig[this.index]
    if (currentLayer) {
      const layerItems = this.props.config.layersConfig.asMutable({
        deep: true
      })
        ...
        <div className='widget-setting-table'>
          <SettingSection className='border-0'>
            <SettingRow>
              <Button
                ...
                type='primary'
                onClick={() => {
                  this.onShowLayerPanel(config.layersConfig.length)
                }}
              >
          ...
          {config.layersConfig.length !== 0 &&
            <SettingSection className='pt-0'>
              <div className='filter-items-container mt-2'>
                {config.layersConfig
                  .asMutable()
                  .map((item: ImmutableObject<LayersConfig>, index: number) => {
                    return (
                      <div
                        key={index}
                        className={classNames(
                          'filter-item',
                          showLayerPanel && this.index === index
                            ? 'filter-item-active'
                            : ''
                        )}
                      >
                        <div
                          className='filter-item-name flex-grow-1'
                          onClick={() => {
                        ...
                        <Button
                          ...
                          icon
                          type='tertiary'
                          onClick={() => this.removeLayer(index)}
                        >
          ...
          >
            <LayerConfig
              ...
              theme={theme}
              useDataSource={
                this.dsHash[this.index] ? this.dsHash[this.index] : null
              }
