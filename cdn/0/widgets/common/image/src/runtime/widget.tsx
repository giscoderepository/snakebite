...
import {
  ...
  css, jsx, AllWidgetProps, DataRecord, ReactResizeDetector, IMExpression, ExpressionPartType, LinkTo, LinkResult, AttachmentInfo, polished, IMUrlParameters, IMState
} from 'jimu-core'
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  render () {
    ...
    if (linkType == LinkType.Page || linkType == LinkType.Dialog || linkType == LinkType.View) {
      renderResult = (
        <div className='widget-image-container jimu-widget'><Link to={linkTo} target={target} queryObject={this.props.queryObject}>{imageContent}
        </Link>
        </div>
      )
    } else if (linkType == LinkType.WebAddress) {
      renderResult = (
        <div className='widget-image-container jimu-widget'><Link to={linkTo} target={target}>{imageContent}
        </Link>
        </div>
      )
    } else {
