...
const clearSelectionIcon = require('jimu-ui/lib/icons/uncheck-all.svg')
...
export class Widget extends React.PureComponent<ListProps, ListStates> {
  ...
  updateListInBuilder = (preProps, preState) => {
    ...
    let hideCardTool = this.layoutPropertiesChange(preProps)
  ...
  layoutPropertiesChange = (preProps): boolean => {
    ...
    const isDirectionChange = config.direction !== preProps.config.direction
    const isEqualCardSizeByDirection = !listUtils.isEqualCardSizeByDirection(
      oldCardSize,
      currentCardSize,
      config.direction
    ) // for change template
    if (
      isDirectionChange ||
      isEqualCardSizeByDirection ||
      isWidgetPositionChange
    ) {
      hideCardTool = true
      if (isEqualCardSizeByDirection) {
        const newState = {
          currentCardSize
        }
        refreshList = true
  ...
  updateScrollContentSize = preConfig => {
    const { config } = this.props
    if (
      config.direction !== preConfig.direction ||
      config.space !== preConfig.space
    ) {
  ...
  onResize = (width, height) => {
    ...
    if (
      cardSize.width < LIST_CARD_MIN_SIZE ||
      cardSize.height < LIST_CARD_MIN_SIZE
    ) {
      return
    }
    this.setState(
    ...
    this.setListDivSize()
    if (
      listUtils.isEqualCardSizeByDirection(
        cardSize,
        oldCardSize,
        config.direction
      )
  ...
  renderListTopTools = (ds: DataSource, queryStatus?: DataSourceStatus, selectRecords?: DataRecord[]) => {
    ...
    const dataName = this.formatMessage('listDataActionLable', { layer: (ds as any)?.layerDefinition?.name || '' })
    const LIST_TOOL_MIN_SIZE = this.checkIsShowDataAction() ? LIST_TOOL_MIN_SIZE_DATA_ACTION : LIST_TOOL_MIN_SIZE_NO_DATA_ACTION
    return (
      <div className='datasource-tools w-100'css={getTopToolStyle(this.props, showTopTools)}>
        <div className="d-flex align-items-center">
          ...
          {(ds && this.checkIsShowDataAction()) && <div className={classNames('list-data-action', {'m-left': listWidth < LIST_TOOL_MIN_SIZE})} data-testid="data-action">
            <DataActionDropDown
              dataName={dataName}
              type='tertiary'
  ...
  getPageSize = () => {
    ...
    const recordSizePerPage = Math.max(
      listUtils.getPageSize(widgetRect, listHeight, this.props),
      1
    )
  ...
  renderTopRightTools = (ds: DataSource, queryStatus?: DataSourceStatus) => {
    ...
    const { config, theme, id, appMode, isRTL } = this.props
    ...
    const showSelectedOnlyIconRtl = isRTL ? showSelectedIconRTL : showSelectedOnlyIcon
    return (
      <div
      ...
      >
        ...
        {listUtils.showDisplaySelectedOnly(this.props) && (
          ...
          >
            <Icon
              icon={showSelectionOnly ? showAllIcon : showSelectedOnlyIconRtl}
              size={16}
            />
          </Button>
        )}
        {listUtils.showClearSelected(this.props) && (
          ...
          >
            <Icon icon={clearSelectionIcon} size={16} />
          </Button>
        )}
      </div>
    )
  ...
  checkIsShowDataAction = (): boolean => {
    ...
    return enableDataAction && this.isDsConfigured()
  }
  ...
  renderList = (ds?: QueriableDataSource, dsInfo?: IMDataSourceInfo) => {
    ...
    const listStyleOption = {
      ...
      showTopTools: (showTopTools || this.checkIsShowDataAction()),
    ...
    return (
      ...
      >
        {/* render top tool */}
        {(showTopTools || this.checkIsShowDataAction()) && this.renderListTopTools(ds, queryStatus, selectRecords)}
        {/* render list */}
        <List
        ...
        {window.jimuConfig.isInBuilder &&
          this.isEditing() &&
          !(!showLoading && (!records || records.length < 1)) && (
            <div className='editing-mask-list' />
          )}
        {!showLoading && (!records || records.length < 1) && (
          <div className='tip-mask-list text-center'>
            {this.formatMessage('listNoData')}
          </div>
        )}
        {/* {(showLoading) && <Loading type={LoadingType.Secondary}/>} */}
        {queryStatus == DataSourceStatus.NotReady && <div className='placeholder-alert-con'>
          <_Alert
            form='tooltip'
            size='small'
            type='warning'
  ...
  getListMaxSize = () => {
    let maxSize = Immutable({
      ...
      maxHeight: this.bodySize.scrollHeight
    })
    ...
    if (type == LayoutType.ColumnLayout && isWidthAuto) {
      maxSize = maxSize.set('maxWidth', this.bodySize.clientWidth)
    }
    if ((type as any) === 'ROW' && isHeightAuto) {
  ...
  itemSize = index => {
    ...
    const size =
      config.direction === DirectionType.Horizon
        ? currentCardSize.width + config.space
